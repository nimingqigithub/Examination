...
public class ExcelUtils {
    ...
    public static List transferExcelToEntity(int rowStart, String fileExtension, InputStream inputStream, Map<String, Integer> map, Class clas) throws IOException {
        ...
        //获取成员变量Map<name,type>
        Map<String, Class> fieldMap = new HashedMap();
        for (int i = 0; i < fields.length; i++) {
            fieldMap.put(fields[i].getName(), fields[i].getType());
        }
        try {
            //构造excel数据对象
            if (StringUtils.equalsIgnoreCase(EXTENSION_XLS, fileExtension)) {
                workbook = new HSSFWorkbook(inputStream);
            } else if (StringUtils.equalsIgnoreCase(EXTENSION_XLSX, fileExtension)) {
            ...
            for (int numSheet = 0; numSheet < workbook.getNumberOfSheets(); numSheet++) {
                ...
                for (int rowIndex = rowStart; rowIndex <= lastRowIndex; rowIndex++) {
                    ...
                    if (StringUtils.isNull(getCellValue(currentRow.getCell(1), true)))
                        continue;
                    for (String key : map.keySet()) {
                        ...
                        Method setMethod = clas.getMethod(StringUtils.getStr(setMethodName), fieldType);
                        //获取参数类型
